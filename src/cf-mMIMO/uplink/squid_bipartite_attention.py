...
# class BipartiteSQUIDGNNPowerControl_Attention(nn.Module):
#     def __init__(
#         self,
#         ap_in=1,
#         ue_in=1,
...
#         self.ap_pos_embedding = nn.Linear(1, pqc_dim)
#         self.ue_pos_embedding = nn.Linear(1, pqc_dim)

#         # Learned sampler scores
#         self.ue2ap_score = MLP(
#             [3 * pqc_dim, hidden_dim, hidden_dim, 1],
#             dropout=dropout,
#         )

#         self.ap2ue_score = MLP(
#             [3 * pqc_dim, hidden_dim, hidden_dim, 1],
#             dropout=dropout,
#         )
...
#             MLP([pqc_dim + self.q_out_dim, hidden_dim, pqc_dim], dropout=dropout)
#             for _ in range(mp_layers)
#         ])
...
#     def _add_positional_embedding(self, ap_h, ue_h):
...
#         ap_pos = torch.linspace(
#             0.0, 1.0, num_ap,
#             device=ap_h.device,
#             dtype=ap_h.dtype,
#         ).view(-1, 1)

#         ue_pos = torch.linspace(
#             0.0, 1.0, num_ue,
#             device=ue_h.device,
#             dtype=ue_h.dtype,
#         ).view(-1, 1)

#         ap_h = ap_h + self.ap_pos_embedding(ap_pos)
#         ue_h = ue_h + self.ue_pos_embedding(ue_pos)
...
#         ue2ap_score = self.ue2ap_score(score_input).squeeze(-1)
#         ap2ue_score = self.ap2ue_score(score_input).squeeze(-1)
...
#         ap_h, ue_h = self._add_positional_embedding(ap_h, ue_h)
